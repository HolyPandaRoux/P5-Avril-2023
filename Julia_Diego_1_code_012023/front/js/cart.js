...
/* Verify the Item Quantity so it can't be negative or superior to 100 and display a message if so*/
function verifyQuantity(quantity) {
    if (quantity < 1 || quantity > 100) {
        alert("Le nombre d'article doit être entre 1 et 100.");
        return false;
    }
    return true;
}
    ...
    productsPanier.forEach(async (cartProduct) => {
        await fetch("http://localhost:3000/api/products/" + cartProduct.id)
            ...
            .then(product => {
                ...
                // total price
                if (verifyQuantity(cartProduct.quantity)) {
